...
class ProductView(View):
	def get(self, request):
		...
		return render(request, 'app/home.html', {'watches':watches, 'laptops':laptops, 'mobiles':mobiles,'cameras':cameras, 'totalitem':totalitem})
...
def watch(request, data=None):
	...
	return render(request, 'app/watch.html', {'watches':watch, 'totalitem':totalitem})
...
def laptop(request, data=None):
	...
	return render(request, 'app/laptop.html', {'laptops':laptop, 'totalitem':totalitem})
...
def camera(request, data=None):
	...
	elif data == 'Sony' or data == 'Canon':
			camera = Product.objects.filter(category='C').filter(brand=data)
	elif data == 'below':
			camera = Product.objects.filter(category='C').filter(discounted_price__lt=40000)
	elif data == 'above':
			camera = Product.objects.filter(category='C').filter(discounted_price__gt=40000)
	return render(request, 'app/camera.html', {'cameras':camera, 'totalitem':totalitem})
